...
On a cloud-managed instance, people don't claim the instance the way a self-hosted operator does — they arrive from the Paperclip Cloud side carrying a *stack role*: `owner`, `admin`, `member`, or `support`. By default none of those roles brings instance admin with it; the control plane owns identity, and the instance never writes an instance-admin role for them.
...
- **Nothing is written down.** Paperclip works the elevation out fresh on every request, at the point where the instance trusts the identity Paperclip Cloud hands it. No instance-admin role row is ever created. Turn the feature off and the owner drops back to company scope on the very next request, with nothing to clean up — and because only that computed check counts here, a leftover instance-admin row sitting in the database elevates nobody on a cloud-managed instance. If the instance can't read its own settings for some reason, it assumes the feature is off: the safe answer wins.
- **The switch can live on the cloud side.** The flag is `enableOwnerInstanceAdmin`, listed as **Owner Instance Admin** on the [Experimental](../experimental/overview.md) page. It's on by default for cloud-managed instances, and it's one of the flags Paperclip Cloud can set for a whole fleet — so you may find it wearing the *Managed by Paperclip Cloud* lock badge rather than being yours to flip.
...
- **Platform-provisioned environments.** Environments the platform created can't be updated or deleted (`environment_platform_managed`). Their environment variables and credential-shaped config keys are never shown to anyone, admin or not; the structural details — provider, image, template, region — stay visible so the environment still renders properly.
